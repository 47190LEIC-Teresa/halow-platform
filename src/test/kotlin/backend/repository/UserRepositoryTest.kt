...
class UserRepositoryTest {
    ...
    fun `should save user and generate id`() {
        val saved = userRepository.saveAndFlush(buildUser())
    ...
    fun `should find user by username`() {
        userRepository.saveAndFlush(buildUser(username = "geronimo"))

        val found = userRepository.findByUsername("geronimo")
    ...
    fun `should return null when username does not exist`() {
        val found = userRepository.findByUsername("missing_user")
    ...
    fun `should update user fields`() {
        ...
        val updated = userRepository.findById(saved.id!!).orElseThrow()
    ...
    fun `should fail when saving duplicate username`() {
        userRepository.saveAndFlush(buildUser(username = "same_user", email = "one@test.com"))

        assertThrows<DataIntegrityViolationException> {
            userRepository.saveAndFlush(buildUser(username = "same_user", email = "two@test.com"))
        }
    }

    @Test
    fun `should fail when saving duplicate email`() {
        userRepository.saveAndFlush(buildUser(username = "user_a", email = "same@test.com"))

        assertThrows<DataIntegrityViolationException> {
            userRepository.saveAndFlush(buildUser(username = "user_b", email = "same@test.com"))
        }
    }

    @Test
    fun `should save user with null last access`() {
        val saved = userRepository.saveAndFlush(buildUser(lastAccess = null))
