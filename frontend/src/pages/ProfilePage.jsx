...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        authFetch("/api/user", token)
            .then(async (res) => {
                if (!res.ok) {
                    throw await parseApiError(res, "Failed to load profile");
                }
                return res.json();
            })
            .then((user) => {
            ...
            .catch((err) => {
                const apiError = normalizeError(err, "Failed to load profile");
                ...
                showApiAlert(showAlert, apiError, "Profile load failed");
            });
    }, [authFetch, token, clearAlert, showAlert]);
    ...
    async function handleSave(e) {
        ...
        try {
            ...
            if (!res.ok) {
                throw await parseApiError(res, "Failed to update profile");
            }
            ...
            setMessage("");
            const apiError = normalizeError(err, "Failed to update profile");
            ...
            showApiAlert(showAlert, apiError, "Profile update failed");
        }
    }
